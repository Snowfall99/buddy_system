...
struct buddy2* buddy2_new (int size) {
    ...
    if(size<1 || !IS_POWER_OF_2(size)) return NULL;
...
void buddy2_destroy(struct buddy2* self) { FREE(self); }
...
int buddy2_alloc(struct buddy2* self, int size) {
    ...
    if(self->longest[index] < size) return -1;

    for(node_size=self->size; node_size != size; node_size /= 2) {
        if(self->longest[LEFT_LEAF(index)] >= size) index = LEFT_LEAF(index);
        else index = RIGHT_LEAF(size);
    }
...
void buddy2_free(struct buddy2* self, int offset) {
    ...
    while(index) {
        ...
        if(left_longest + right_longest == node_size) self->longest[index] = node_size;
        else self->longest[index] = MAX(left_longest, right_longest);
    }
}
...
int buddy2_size(struct buddy2* self, int offset) {
    ...
    node_size = 1;
    for(index=offset+self->size-1; self->longest[index]; index=PARENT(index)) node_size *= 2;
